#!/usr/bin/env python3
"""
Test Actual New Room Sofa Replacement
=====================================

Test the sofa replacement system with the actual new living room image provided by the user.
"""
...
# Configuration
OUTPUT_DIR = "actual_new_room_replacement"
ROOM_IMAGE = "new_room.jpg"  # The actual new room image
...
def check_room_image():
    """Check if the new room image exists"""
    if not os.path.exists(ROOM_IMAGE):
        print(f"❌ Room image not found: {ROOM_IMAGE}")
        print("Please save the new living room image as 'new_room.jpg' in the current directory.")
        return False
    
    # Load and check the image
    try:
        img = cv2.imread(ROOM_IMAGE)
        if img is None:
            print(f"❌ Could not load room image: {ROOM_IMAGE}")
            return False
        
        h, w = img.shape[:2]
        print(f"✅ Room image loaded: {w}x{h} pixels")
        return True
...
def detect_sofa_in_actual_room(room_image_path):
    """Detect the sofa in the actual new room image"""
    print("🎯 Detecting sofa in actual new room...")
    ...
    print("⚠️ Using manual sofa area definition for actual new room")
    
    # Based on the new room image description, the sofa is:
...
def replace_sofa_naturally(clean_room_path, cutout_path, shadow_path, positioning):
    ...
    # Save result
    result_path = os.path.join(OUTPUT_DIR, "actual_new_room_sofa_replacement.png")
    result_img.save(result_path)
    
    print(f"✅ Actual new room sofa replacement saved: {result_path}")
...
def main():
    """Main actual new room sofa replacement"""
    print("🛋️  ACTUAL NEW ROOM SOFA REPLACEMENT")
    print("=" * 50)
    
    # Check if the room image exists
    if not check_room_image():
        return
    ...
    try:
        ...
        # Step 2: Detect sofa in actual new room
        mask_path, mask, sofa_bbox = detect_sofa_in_actual_room(ROOM_IMAGE)
        ...
        print("\n🎉 ACTUAL NEW ROOM SOFA REPLACEMENT COMPLETE!")
        print("=" * 50)
        ...
        print(f"❌ Actual new room sofa replacement failed: {e}")
        raise
